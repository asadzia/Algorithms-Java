  ...
  int matchingPairs(String s, String t) {
    ...
    for (int i = 0; i < s.length(); ++i) {
      ...
      if (!found_swap) {
        ...
        List<Integer> x = mp.getOrDefault(a_s[i], null);
        
        if (x != null) {
          
          if (x.get(0) == (int)a_t[i]) {
            // then swap
            found_swap = true;
            Character y = a_s[i];
            a_s[i] = a_s[x.get(1)];
    ...
    // Handle edge case when the entire strings are same
    if (mp.size() == 0) {
      
      return a_s.length - 2;
    }
    
    return a_s.length - mp.size() + (found_swap ? 2 : 0);
  }
